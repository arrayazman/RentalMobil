...
<div class="row">
    <div class="col-md-6">
        <form action="/sewa/update/<?= $sewa['id_sewa'] ?>" method="post">
            <?= csrf_field() ?>
            ...
            <div class="mb-3">
                ...
                <select class="form-select <?= (session('errors.pelanggan_id')) ? 'is-invalid' : '' ?>" id="pelanggan_id" name="pelanggan_id">
                    ...
                    <?php foreach ($pelanggans as $p) : ?>
                        <option value="<?= $p['id_pelanggan'] ?>" <?= (old('pelanggan_id', $sewa['pelanggan_id']) == $p['id_pelanggan']) ? 'selected' : '' ?>><?= $p['nama'] ?> (<?= $p['no_ktp'] ?>)</option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-select <?= (session('errors.mobil_id')) ? 'is-invalid' : '' ?>" id="mobil_id" name="mobil_id">
                    ...
                    <?php foreach ($mobils as $m) : ?>
                        <option value="<?= $m['id_mobil'] ?>" <?= (old('mobil_id', $sewa['mobil_id']) == $m['id_mobil']) ? 'selected' : '' ?>><?= $m['merk'] ?> - <?= $m['tipe'] ?></option>
                    <?php endforeach; ?>
                </select>
                <div class="invalid-feedback">
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control <?= (session('errors.tgl_sewa')) ? 'is-invalid' : '' ?>" id="tgl_sewa" name="tgl_sewa" value="<?= old('tgl_sewa', $sewa['tgl_sewa']) ?>">
                 <div class="invalid-feedback">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control <?= (session('errors.lama_sewa')) ? 'is-invalid' : '' ?>" id="lama_sewa" name="lama_sewa" value="<?= old('lama_sewa', $sewa['lama_sewa']) ?>">
                 <div class="invalid-feedback">
