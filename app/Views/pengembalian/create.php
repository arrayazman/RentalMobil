    ...
    <div class="col-md-6">
        <form action="/pengembalian/store" method="post">
            ...
            <div class="mb-3">
                ...
                <select class="form-select <?= (session('errors.sewa_id')) ? 'is-invalid' : '' ?>" id="sewa_id" name="sewa_id">
                    ...
                    <?php foreach ($sewas as $s) : ?>
                        <option value="<?= $s['id_sewa'] ?>" <?= (old('sewa_id') == $s['id_sewa']) ? 'selected' : '' ?>><?= $s['nama'] ?> - <?= $s['merk'] ?> (<?= $s['no_polisi'] ?>)</option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control <?= (session('errors.tgl_kembali')) ? 'is-invalid' : '' ?>" id="tgl_kembali" name="tgl_kembali" value="<?= old('tgl_kembali', date('Y-m-d')) ?>">
                 <div class="invalid-feedback">
